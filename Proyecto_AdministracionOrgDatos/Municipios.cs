...
namespace Proyecto_AdministracionOrgDatos
{
    internal class Municipios
    {
        ...
        private Dictionary<string, List<string>> municipiosPorEstado;

        public Municipios(ComboBox cmbEstadoNac, ComboBox cmbMunicipio)
        {
            this.cmbEstadoNac = cmbEstadoNac;
            this.cmbMunicipio = cmbMunicipio;

            // Inicializar el diccionario de municipios por estado
            municipiosPorEstado = new Dictionary<string, List<string>>();

            // Llenar ComboBox de estados
            LlenarComboBoxEstados();

            // Manejar el evento de cambio de selección del ComboBox de estados
            cmbEstadoNac.SelectedIndexChanged += CmbEstadoNac_SelectedIndexChanged;
        }

        public void LlenarComboBoxEstados()
        {
            // Agregar los estados a ComboBox de estados
            ...
            // Agregar los municipios correspondientes a cada estado al diccionario
            municipiosPorEstado.Add("Aguascalientes", new List<string> { "Aguascalientes", "Asientos", "Calvillo", "Cosío", "Jesús María", "Pabellón de Arteaga", "Rincón de Romos", "San José de Gracia", "Tepezalá", "El Llano", "San Francisco de los Romo" });
    ...
    "Cuauhtémoc", "Chalchihuites", "Fresnillo", "Trinidad García de la Cadena", "Genaro Codina", "General Enrique Estrada",
        ...
        {
            // Limpiar el ComboBox de municipios
            cmbMunicipio.Items.Clear();

            // Obtener los municipios correspondientes al estado seleccionado
            if (municipiosPorEstado.ContainsKey(estado))
            {
                List<string> municipios = municipiosPorEstado[estado];
                // Agregar los municipios al ComboBox de municipios
